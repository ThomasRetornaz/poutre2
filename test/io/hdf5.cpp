...
TEST_CASE("load/save 2D gray 8 hdf5", "[io]")
{
 ...
 0 0 0 0 0
  ...
  fs::path const image_path = tempDir / "write_test_GUINT8.h5";
...
TEST_CASE("load/save 2D RGB 8 hdf5", "[io]")
{
  fs::path const tempDir = "POUTRE_NRT_IO_TMP_DIR";
  if (!fs::is_directory(tempDir))
  {
...
0 0 255 228 135 255 255 255 255
  ...
  fs::path const image_path = tempDir / "write_test_GUINT8_RGB.h5";
  auto writer = poutre::io::ImageWriter().SetPath(image_path.string());
  writer.Write(*img_in);
...
TEST_CASE("load/save 2D RGBA 8 hdf5", "[io]")
{
  fs::path const tempDir = "POUTRE_NRT_IO_TMP_DIR";
  if (!fs::is_directory(tempDir))
  {
...
0 0 255 255 228 135 255 255 255 255 255 255
  ...
  fs::path const image_path = tempDir / "write_test_GUINT8_RGBA.h5";
  auto writer = poutre::io::ImageWriter().SetPath(image_path.string());
  writer.Write(*img_in);
