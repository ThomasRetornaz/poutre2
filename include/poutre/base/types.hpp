...
template<class valuetype, std::ptrdiff_t Rank> class compound_type;
template<class valuetype> class compound_type<valuetype, 3>;


template<class valuetype, std::ptrdiff_t Rank> class compound_type// NOLINT
{
  // Static assertion
  static_assert(Rank > 0, "compound_type requires a Rank>0");
  static_assert(std::is_arithmetic<valuetype>::value, "compound_type only support arithmetic type");// or use concept ?
...
public:
  ...
  constexpr compound_type(const self_type &rhs) = default;

  constexpr self_type &operator=(const self_type &rhs) = default;

  constexpr compound_type(self_type &&rhs) = default;
...
{
  static constexpr ptrdiff_t dim = 1;
};

//! Helper to retrieve dim off compound
template<class value_type, std::ptrdiff_t Rank> struct get_dim<compound_type<value_type, Rank>>
{
  static constexpr ptrdiff_t dim = Rank;
};
...
public:
  ...
  constexpr explicit compound_type(const self_type &rhs) = default;

  constexpr self_type &operator=(const self_type &rhs) = default;

  constexpr explicit compound_type(self_type &&rhs) = default;
...
extern template class compound_type<pUINT8, 3>;
extern template class compound_type<pINT32, 3>;
extern template class compound_type<pFLOAT, 3>;
extern template class compound_type<pINT64, 3>;
extern template class compound_type<pDOUBLE, 3>;

extern template class compound_type<pUINT8, 4>;
extern template class compound_type<pINT32, 4>;
extern template class compound_type<pFLOAT, 4>;
extern template class compound_type<pINT64, 4>;
extern template class compound_type<pDOUBLE, 4>;
