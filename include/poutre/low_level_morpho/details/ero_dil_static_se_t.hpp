...
void t_LineBufferShiftRight(const T *i_viewlinein, scoord lenghtline, scoord nbshift, T paddValue, T *o_viewlineout)
{
  ...
  if (nbshift >= lenghtline) {
    ...
    for (i = 0; i < lenghtline; i++) { lineout[i] = paddValue; }
    return;
  }

  // pad left border before shift
  for (i = 0; i < nbshift; i++) { lineout[i] = paddValue; }
  // shift to right
...
void t_3DCopyOneSliceTo2D(T *imIn3d,
  ...
  const std::size_t wxSize,
  const std::size_t wySize,
...
void t_LineBufferShiftLeft(const T *i_viewlinein, scoord lenghtline, scoord nbshift, T paddValue, T *o_viewlineout)
{
  ...
  // then pad right border
  for (i = lenghtline - nbshift; i < lenghtline; i++) { lineout[i] = paddValue; }
}
...
template<poutre::se::Common_NL_SE nl_static, typename T1, typename T2, class BinOp>
struct t_ErodeDilateDispatcher<nl_static,
  T1,
  T2,
  2,
  poutre::details::av::array_view,
  poutre::details::av::array_view,
  BinOp>
{
  static_assert(2 == poutre::se::details::static_se_traits<nl_static>::rank, "SE and view have not the same Rank==2");
  // cppcheck-suppress constParameterReference
  void operator()(const poutre::details::av::array_view<const T1, 2> &i_vin,
    const poutre::details::av::array_view<T2, 2> &o_vout)
  {
    POUTRE_CHECK(i_vin.size() == o_vout.size(), "Incompatible views size");
    ...
    for (ptrdiff_t y = 0; y < half_size; ++y) {
      for (ptrdiff_t x = 0; x < xsize; ++x) {
        t_ErodeDilateIterateBorderArrayView2DHelper<nl_static, T1, T2, BinOp>(
          i_vinbeg, o_voutbeg, xsize, ysize, istride[0], x, y);
      }
    }
...
struct t_ErodeDilateDispatcher<se::Common_NL_SE::SESquare2D,
  TIn,
  TOut,
  2,
  poutre::details::av::array_view,
  ...
  HelperOp>
{
  void operator()(const poutre::details::av::array_view<const TIn, 2> &i_vin,
    const poutre::details::av::array_view<TOut, 2> &o_vout) const
  {
    POUTRE_CHECK(i_vin.size() == o_vout.size(), "Incompatible views size");
    auto ibd = i_vin.bound();
    ...
    tmpBuffer tempLine1(static_cast<std::size_t>(xsize));
    tmpBuffer tempLine2(static_cast<std::size_t>(xsize));
    tmpBuffer tempLine3(static_cast<std::size_t>(xsize));
    lineView bufTempLine(tempLine.data());
    lineView bufTempLine1(tempLine1.data());
    lineView bufTempLine2(tempLine2.data());
    lineView bufTempLine3(tempLine3.data());
...
template<typename TIn, typename TOut, class HelperOp>
struct t_ErodeDilateDispatcher<se::Common_NL_SE::SECross2D,
  TIn,
  TOut,
  2,
  poutre::details::av::array_view,
...
template<typename TIn, typename TOut, class HelperOp>
struct t_ErodeDilateDispatcher<se::Common_NL_SE::SESegmentX2D,
  TIn,
  TOut,
  2,
  poutre::details::av::array_view,
...
template<typename TIn, typename TOut, class HelperOp>
struct t_ErodeDilateDispatcher<se::Common_NL_SE::SESegmentY2D,
  TIn,
  TOut,
  2,
  poutre::details::av::array_view,
...
struct Dilate2DFor3DSquare
{
public:
  using LineOp = LineBufferShiftAndArithDilateHelperOp<T>;

  static void
    ApplyArith(const ViewIn<const T, 2> &i_vin1, const ViewIn<const T, 2> &i_vin2, const ViewOut<T, 2> &o_vout)
...
template<typename TIn, typename TOut, class HelperOp>
struct t_ErodeDilateDispatcher<se::Common_NL_SE::SESquare3D,
  TIn,
  TOut,
  3,
  poutre::details::av::array_view,
  ...
  HelperOp>
{
  void operator()(const poutre::details::av::array_view<const TIn, 3> &i_vin,
    const poutre::details::av::array_view<TOut, 3> &o_vout) const
  {
    POUTRE_CHECK(i_vin.size() == o_vout.size(), "Incompatible views size");
    auto ibd = i_vin.bound();
    auto obd = o_vout.bound();// NOLINT
    auto istride = i_vin.stride();
    auto ostride = o_vout.stride();
    const auto xsize = static_cast<std::size_t>(ibd[0]);
    const auto ysize = static_cast<std::size_t>(ibd[1]);
    const auto zsize = static_cast<std::size_t>(ibd[2]);
    POUTRE_CHECK(ibd == obd, "bound not compatible");
    POUTRE_CHECK(istride == ostride, "stride not compatible");
...
template<typename TIn, typename TOut, class HelperOp>
struct t_ErodeDilateDispatcher<se::Common_NL_SE::SECross3D,
  TIn,
  TOut,
  3,
  poutre::details::av::array_view,
  ...
  ViewOut<TOut, Rank> &o_vout)
{
  ...
  if constexpr (Rank == 1) {
    ...
    case poutre::se::Common_NL_SE::SESegmentX1D: {
      t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESegmentX1D, TIn, TOut, Rank, ViewIn, ViewOut, BinOp>
        dispatcher;
      dispatcher(i_vin, o_vout);
    } break;
    default: {
  ...
  if constexpr (Rank == 2) {
    ...
    case poutre::se::Common_NL_SE::SESegmentX2D: {
      // t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESegmentX2D, TIn,TOut, Rank, ViewIn, ViewOut, BinOp>
      // dispatcher; dispatcher(i_vin, o_vout);
      using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
      t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESegmentX2D, TIn, TOut, Rank, ViewIn, ViewOut, LineOp>
        dispatcher;
      dispatcher(i_vin, o_vout);
    } break;
    case poutre::se::Common_NL_SE::SESegmentY2D: {
      // t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESegmentY2D, TIn,TOut, Rank, ViewIn, ViewOut, BinOp>
      // dispatcher; dispatcher(i_vin, o_vout);
      using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
      t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESegmentY2D, TIn, TOut, Rank, ViewIn, ViewOut, LineOp>
        dispatcher;
      dispatcher(i_vin, o_vout);
    } break;
    case poutre::se::Common_NL_SE::SESquare2D: {
      // t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESquare2D, TIn,TOut, Rank, ViewIn, ViewOut, BinOp>
      // dispatcher; dispatcher(i_vin, o_vout);
      using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
      t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESquare2D, TIn, TOut, Rank, ViewIn, ViewOut, LineOp>
        dispatcher;
      dispatcher(i_vin, o_vout);
    } break;
    case poutre::se::Common_NL_SE::SECross2D: {
      // t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SECross2D, TIn,TOut, Rank, ViewIn, ViewOut, BinOp>
      // dispatcher; dispatcher(i_vin, o_vout);
      using LineOp = LineBufferShiftAndArithDilateHelperOp<TIn>;
  ...
  if constexpr (Rank == 3) {
    ...
    case poutre::se::Common_NL_SE::SECross3D: {
      // t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SECross3D, TIn,TOut, Rank, ViewIn, ViewOut, BinOp>
      // dispatcher; dispatcher(i_vin, o_vout);
      using Helper2DOp = Dilate2DFor3DCross<TIn, ViewIn, ViewOut>;
      t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SECross3D, TIn, TOut, Rank, ViewIn, ViewOut, Helper2DOp>
        dispatcher;
      dispatcher(i_vin, o_vout);
    } break;
    case poutre::se::Common_NL_SE::SESquare3D: {
      using Helper2DOp = Dilate2DFor3DSquare<TIn, ViewIn, ViewOut>;
      t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESquare3D, TIn, TOut, Rank, ViewIn, ViewOut, Helper2DOp>
        dispatcher;
      dispatcher(i_vin, o_vout);
    } break;
    case poutre::se::Common_NL_SE::SESegmentX3D: {
      t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESegmentX3D, TIn, TOut, Rank, ViewIn, ViewOut, BinOp>
        dispatcher;
      dispatcher(i_vin, o_vout);
    } break;
    case poutre::se::Common_NL_SE::SESegmentY3D: {
      t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESegmentY3D, TIn, TOut, Rank, ViewIn, ViewOut, BinOp>
        dispatcher;
      dispatcher(i_vin, o_vout);
    } break;
    case poutre::se::Common_NL_SE::SESegmentZ3D: {
      t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESegmentZ3D, TIn, TOut, Rank, ViewIn, ViewOut, BinOp>
        dispatcher;
      dispatcher(i_vin, o_vout);
    } break;
    default: {
  ...
  ViewOut<TOut, Rank> &o_vout)
{
  ...
  if constexpr (Rank == 3) {
    ...
    case poutre::se::Common_NL_SE::SESquare3D: {
      // t_ErodeDilateDispatcher<poutre::se::Common_NL_SE::SESquare3D, TIn,TOut, Rank, ViewIn, ViewOut, BinOp>
      // dispatcher; dispatcher(i_vin, o_vout);
...
void t_Erode(const poutre::details::image_t<T1, Rank> &i_img,
  poutre::se::Common_NL_SE nl_static,
  poutre::details::image_t<T2, Rank> &o_img)
{
