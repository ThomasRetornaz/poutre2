...
struct t_ErodeDilateXOpLineDispatcher<T, T, 1, poutre::details::av::array_view, poutre::details::av::array_view, BinOp>
{
  void operator()(const poutre::details::av::array_view<const T, 1> &i_vin,
    ptrdiff_t size_line_segment,
    const poutre::details::av::array_view<T, 1> &o_vout) const
  {
    BinOp op;
    ...
    if (size_line_segment <= 0) {
      std::memcpy(reinterpret_cast<void *>(o_vout.data()),
        reinterpret_cast<const void *>(i_vin.data()),
        sizeof(T) * i_vin.size());
      return;
    }
    ...
    van_herck_1d(f.data() + size_line_segment,
      g.data() + size_line_segment,
      h.data() + size_line_segment,
      size_line,
      size_line_segment,
      op);
...
template<typename T, class BinOp>
struct t_ErodeDilateXOpLineDispatcher<T, T, 2, poutre::details::av::array_view, poutre::details::av::array_view, BinOp>
{
  void operator()(const poutre::details::av::array_view<const T, 2> &i_vin,
    ptrdiff_t size_line_segment,
    poutre::details::av::array_view<T, 2> &o_vout) const
  {
    BinOp op;
    POUTRE_CHECK(i_vin.size() == o_vout.size(), "Incompatible views size");
    auto ibd = i_vin.bound();
...
void t_ErodeX(const poutre::details::image_t<TIn, Rank> &i_img,
  ptrdiff_t size_segment,
...
template<typename TIn, typename TOut, ptrdiff_t Rank>
void t_DilateX(const poutre::details::image_t<TIn, Rank> &i_img,
  ptrdiff_t size_segment,
  poutre::details::image_t<TOut, Rank> &o_img)
{
...
void t_DilateY(const poutre::details::image_t<TIn, 2> &i_img,
  ptrdiff_t size_segment,
  poutre::details::image_t<TOut, 2> &o_img)
{
...
template<typename TIn, typename TOut>
void t_ErodeY(const poutre::details::image_t<TIn, 2> &i_img,
  ptrdiff_t size_segment,
  poutre::details::image_t<TOut, 2> &o_img)
{
  POUTRE_ENTERING("t_DilateY");
