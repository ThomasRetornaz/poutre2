...
void StoreWithOIIOScalarDispatch(const IInterface &img, const std::string &path, details::StoreWithOIIOOptions const &options, PType ptype)
 {
   switch (ptype) {
     // todo think about bool/binary here
   case PType::PType_GrayUINT8: {
...
void StoreWithOIIO3PlanesDispatch(const IInterface &img, const std::string &path, details::StoreWithOIIOOptions const &options, PType ptype)
 {
   switch (ptype) {
     // todo think about bool/binary here
   case PType::PType_GrayUINT8: {
...
void StoreWithOIIO4PlanesDispatch(const IInterface &img, const std::string &path, details::StoreWithOIIOOptions const &options, PType ptype)
 {
   switch (ptype) {
     // todo think about bool/binary here
   case PType::PType_GrayUINT8: {
  ...
  std::unique_ptr<IInterface> LoadOIIO(const std::string &image_path)
  {
    ...
    if( spec.height > 1 ) {
      dims.push_back(static_cast<std::size_t>(spec.height));
}
    if( spec.width > 1 ) {
      dims.push_back(static_cast<std::size_t>(spec.width));
}
    if( spec.depth > 1 ) {
    ...
    {
    case 1: ctype = CompoundType::CompoundType_Scalar; break;
    case 3: ctype = CompoundType::CompoundType_3Planes; break;
    case 4: ctype = CompoundType::CompoundType_4Planes; break;
    default:
      std::ostringstream error_stream;
      error_stream << " load_image(): Error reading image '";
      error_stream << image_path;
    ...
    {
    case OpenImageIO_v2_4::TypeDesc::BASETYPE::UINT8: ptype = PType::PType_GrayUINT8; break;
    case OpenImageIO_v2_4::TypeDesc::BASETYPE::INT32: ptype = PType::PType_GrayINT32; break;
    case OpenImageIO_v2_4::TypeDesc::BASETYPE::FLOAT: ptype = PType::PType_F32; break;
    case OpenImageIO_v2_4::TypeDesc::BASETYPE::INT64: ptype = PType::PType_GrayINT64; break;
    case OpenImageIO_v2_4::TypeDesc::BASETYPE::DOUBLE: ptype = PType::PType_D64; break;
    default:
      std::ostringstream error_stream;
      error_stream << " load_image(): Error reading image '";
      error_stream << image_path;
  ...
  void DumpOIIO(const std::string &path, const IInterface &i_img) // NOLINT(misc-unused-parameters)
  {
    ...
    if( i_img.GetRank() > 2 ) {
      POUTRE_RUNTIME_ERROR(std::format("For nd images with n>2 use StoreWithHDF5"));
}
